export const translations = {
    ru: {
        nav: {
            services: "Услуги",
            process: "Процесс",
            portfolio: "Портфолио",
            about: "О нас",
            contact: "Оставить заявку"
        },
        seo: {
            title: "Разработка сайтов в Минске под ключ | WebNexum — веб-студия",
            description: "Разработка сайтов в Минске под ключ. Создание веб-приложений, корпоративных сайтов, лендингов и интернет-магазинов. Профессиональная веб-разработка с гарантией качества. WebNexum — ваша веб-студия в Минске."
        },
        hero: {
            title: "Разработка сайтов",
            titleSuffix: "в Минске под ключ",
            description: "WebNexum — веб-студия в Минске. Разрабатываем сайты под ключ, веб-приложения и корпоративные решения. От идеи до поддержки.",
            cta: "Оставить заявку",
            portfolio: "Посмотреть портфолио"
        },
        stats: {
            projects: "Проекты",
            weeks: "Средний срок",
            months: "Поддержка"
        },
        services: {
            title: "Наши услуги",
            description: "Полный цикл разработки сайтов под ключ: анализ, дизайн, разработка, тестирование и поддержка.",
            website: { title: "Разработка сайтов", desc: "Лендинги, корпоративные сайты, интернет-магазины" },
            webapp: { title: "Веб-приложения", desc: "SaaS, панели управления, кастомные решения" },
            mobile: { title: "Мобильные решения", desc: "PWA, интеграции, адаптив" },
            design: { title: "Дизайн & Бренд", desc: "UI/UX, прототипы, айдентика" }
        },
        process: {
            title: "Как мы работаем",
            description: "Прозрачный процесс разработки от первого контакта до запуска проекта.",
            step1: { title: "Обсуждение и анализ", desc: "Изучаем требования, цели проекта и целевую аудиторию. Формируем техническое задание." },
            step2: { title: "Прототипирование и дизайн", desc: "Создаём wireframes и дизайн-макеты с учётом UX/UI best practices. Согласовываем каждый этап." },
            step3: { title: "Разработка", desc: "Пишем чистый, масштабируемый код. Работаем по Agile методологии с регулярными демо." },
            step4: { title: "Тестирование и запуск", desc: "Проводим комплексное тестирование. Готовим к деплою и запускаем проект." },
            step5: { title: "Поддержка и развитие", desc: "Обеспечиваем техническую поддержку и дальнейшее развитие проекта." }
        },
        portfolio: {
            title: "Портфолио",
            description: "Примеры проектов — кликайте для деталей.",
            details: "Подробнее →",
            problem: "Проблема заказчика",
            whatWeDid: "Что мы сделали",
            close: "Закрыть"
        },
        about: {
            why: "Почему выбирают нас",
            whyTitle: "О нас",
            team: { title: "Опытная команда", desc: "Senior/middle разработчики с опытом реализации enterprise задач" },
            process: { title: "Прозрачные процессы", desc: "Гибкий подход (Agile) с регулярными демо и отчетами" },
            support: { title: "Поддержка", desc: "Сопровождение и техподдержка после релиза" },
            aboutTitle: "О нас",
            aboutText: "WebNexum — веб-студия в Минске. Разрабатываем сайты под ключ и веб-приложения. Работаем с компаниями, которые ценят скорость, качество и результат.",
            contact: "Связаться",
            cases: "Кейсы"
        },
        contact: {
            title: "Готовы обсудить проект?",
            description: "Оставьте заявку — опишем шаги и предварительную оценку.",
            email: "Email",
            telegram: "Telegram",
            address: "Адрес",
            addressValue: "Remote / Минск",
            name: "Имя",
            namePlaceholder: "Ваше имя",
            phone: "Телефон",
            phonePlaceholder: "+375 (XX) XXX-XX-XX",
            emailField: "Email",
            emailPlaceholder: "your@email.com",
            projectType: "Тип проекта",
            projectTypes: {
                website: "Сайт-лендинг",
                webapp: "Веб-приложение",
                software: "ПО под ключ",
                other: "Другое"
            },
            message: "Сообщение",
            messagePlaceholder: "Расскажите о вашем проекте...",
            connectMethod: "Способ связи",
            telegramOption: "Telegram",
            phoneOption: "Телефон",
            emailOption: "Email",
            viberOption: "Viber",
            viberPlaceholder: "Номер Viber",
            submit: "Отправить заявку",
            sending: "Отправка...",
            quickContact: "Быстрый контакт",
            quickContactDesc: "Готовы обсудить проект? Оставьте заявку — мы ответим в течение рабочего дня."
        },
        faq: {
            title: "FAQ — Частые вопросы"
        },
        footer: {
            rights: "© 2024 WebNexum. Все права защищены."
        },
        toast: {
            success: "Успешно!",
            error: "Ошибка",
            sent: "Заявка отправлена — спасибо! Мы ответим в ближайшее время.",
            errorMsg: "Не удалось отправить заявку. Пожалуйста, свяжитесь с нами напрямую: support@webnexum.com или @webnexum в Telegram."
        },
        projects: {
            project1: {
                title: "Криптоплатформа — Zignaly",
                desc: "Крипто инвесторская платформа с системой profit sharing.",
                problem: "Заказчик столкнулся с необходимостью добвления функционала в масштабируемую платформу для криптоинвестиций с автоматическим распределением прибыли между инвесторами и трейдерами. Существующее решение не справлялось с нагрузкой и не имело необходимой функциональности.",
                whatWeDid: [
                    "Переработали архитектуру микросервисов для обеспечения масштабируемости",
                    "Реализовали систему автоматического profit sharing с поддержкой множественных стратегий",
                    "Создали админ-панель для управления пользователями и стратегиями",
                    "Интегрировали API криптобирж для автоматической торговли",
                    "Внедрили систему уведомлений и аналитики в реальном времени",
                    "Оптимизировали производительность базы данных для обработки больших объемов транзакций"
                ]
            },
            project2: {
                title: "Страница игры — Vendetta City",
                desc: "Одностраничный лэндинг для ознакомления с игрой Vendetta City.",
                problem: "Необходимо было создать привлекательный лэндинг для новой игры, который бы эффективно конвертировал посетителей в игроков. Требовалось показать уникальные особенности игры и создать атмосферу, соответствующую игровому миру.",
                whatWeDid: [
                    "Разработали современный одностраничный лэндинг с анимациями и интерактивными элементами",
                    "Реализовали адаптивный дизайн для всех устройств",
                    "Интегрировали систему регистрации и авторизации",
                    "Создали секции с геймплейными видео и скриншотами",
                    "Добавили систему предзаказа с интеграцией платежных систем",
                    "Оптимизировали производительность для быстрой загрузки"
                ]
            },
            project3: {
                title: "Логистическая система — Buster GFC",
                desc: "Подсистема управления заказами в автоматизированной логистической системе.",
                problem: "Логистическая компания нуждалась в автоматизации процесса управления заказами. Ручная обработка заказов приводила к ошибкам, задержкам и неэффективному использованию ресурсов. Требовалась система для отслеживания заказов от создания до доставки.",
                whatWeDid: [
                    "Разработали полнофункциональную систему управления заказами",
                    "Реализовали автоматическое распределение заказов по складам и курьерам",
                    "Создали систему отслеживания статусов заказов в реальном времени",
                    "Интегрировали API транспортных компаний для автоматического создания накладных",
                    "Внедрили систему уведомлений для клиентов и сотрудников",
                    "Разработали аналитическую панель для оптимизации логистических процессов"
                ]
            }
        }
    },
    en: {
        nav: {
            services: "Services",
            process: "Process",
            portfolio: "Portfolio",
            about: "About",
            contact: "Contact Us"
        },
        seo: {
            title: "Website Development in Minsk Turnkey | WebNexum — Web Studio",
            description: "Website development in Minsk turnkey. Creating web applications, corporate websites, landing pages and online stores. Professional web development with quality guarantee. WebNexum — your web studio in Minsk."
        },
        hero: {
            title: "Website Development",
            titleSuffix: "in Minsk Turnkey",
            description: "WebNexum — web studio in Minsk. We develop turnkey websites, web applications and corporate solutions. From idea to support.",
            cta: "Get Started",
            portfolio: "View Portfolio"
        },
        stats: {
            projects: "Projects",
            weeks: "Avg. Duration",
            months: "Support"
        },
        services: {
            title: "Our Services",
            description: "Full cycle of turnkey website development: analysis, design, development, testing and support.",
            website: { title: "Website Development", desc: "Landing pages, corporate websites, online stores" },
            webapp: { title: "Web Applications", desc: "SaaS, admin panels, custom solutions" },
            mobile: { title: "Mobile Solutions", desc: "PWA, integrations, responsive" },
            design: { title: "Design & Brand", desc: "UI/UX, prototypes, identity" }
        },
        process: {
            title: "How We Work",
            description: "Transparent development process from first contact to project launch.",
            step1: { title: "Discussion & Analysis", desc: "We study requirements, project goals and target audience. We create technical specifications." },
            step2: { title: "Prototyping & Design", desc: "We create wireframes and design mockups considering UX/UI best practices. We coordinate each stage." },
            step3: { title: "Development", desc: "We write clean, scalable code. We work using Agile methodology with regular demos." },
            step4: { title: "Testing & Launch", desc: "We conduct comprehensive testing. We prepare for deployment and launch the project." },
            step5: { title: "Support & Growth", desc: "We provide technical support and further project development." }
        },
        portfolio: {
            title: "Portfolio",
            description: "Project examples — click for details.",
            details: "Learn More →",
            problem: "Client Problem",
            whatWeDid: "What We Did",
            close: "Close"
        },
        about: {
            why: "Why Choose Us",
            whyTitle: "About Us",
            team: { title: "Experienced Team", desc: "Senior/middle developers with experience in enterprise solutions" },
            process: { title: "Transparent Processes", desc: "Flexible approach (Agile) with regular demos and reports" },
            support: { title: "Support", desc: "Maintenance and technical support after release" },
            aboutTitle: "About Us",
            aboutText: "WebNexum — web studio in Minsk. We develop turnkey websites and web applications. We work with companies that value speed, quality and results.",
            contact: "Contact",
            cases: "Cases"
        },
        contact: {
            title: "Ready to discuss your project?",
            description: "Leave a request — we'll outline the steps and preliminary estimate.",
            email: "Email",
            telegram: "Telegram",
            address: "Address",
            addressValue: "Remote / Minsk",
            name: "Name",
            namePlaceholder: "Your name",
            phone: "Phone",
            phonePlaceholder: "+1 (XXX) XXX-XXXX",
            emailField: "Email",
            emailPlaceholder: "your@email.com",
            projectType: "Project Type",
            projectTypes: {
                website: "Landing Page",
                webapp: "Web Application",
                software: "Custom Software",
                other: "Other"
            },
            message: "Message",
            messagePlaceholder: "Tell us about your project...",
            connectMethod: "Contact Method",
            telegramOption: "Telegram",
            phoneOption: "Phone",
            emailOption: "Email",
            viberOption: "Viber",
            viberPlaceholder: "Viber number",
            submit: "Send Request",
            sending: "Sending...",
            quickContact: "Quick Contact",
            quickContactDesc: "Ready to discuss your project? Leave a request — we'll respond within a business day."
        },
        faq: {
            title: "FAQ — Frequently Asked Questions"
        },
        footer: {
            rights: "© 2024 WebNexum. All rights reserved."
        },
        toast: {
            success: "Success!",
            error: "Error",
            sent: "Request sent — thank you! We'll respond soon.",
            errorMsg: "Failed to send request. Please contact us directly: support@webnexum.com or @webnexum on Telegram."
        },
        projects: {
            project1: {
                title: "Crypto Platform — Zignaly",
                desc: "Crypto investment platform with profit sharing system.",
                problem: "The client needed to add functionality to a scalable crypto investment platform with automatic profit distribution between investors and traders. The existing solution couldn't handle the load and lacked necessary functionality.",
                whatWeDid: [
                    "Redesigned microservices architecture for scalability",
                    "Implemented automatic profit sharing system with support for multiple strategies",
                    "Created admin panel for managing users and strategies",
                    "Integrated crypto exchange APIs for automated trading",
                    "Implemented real-time notifications and analytics system",
                    "Optimized database performance for processing large transaction volumes"
                ]
            },
            project2: {
                title: "Game Page — Vendetta City",
                desc: "Single-page landing for Vendetta City game introduction.",
                problem: "We needed to create an attractive landing page for a new game that would effectively convert visitors into players. We needed to showcase unique game features and create an atmosphere matching the game world.",
                whatWeDid: [
                    "Developed modern single-page landing with animations and interactive elements",
                    "Implemented responsive design for all devices",
                    "Integrated registration and authorization system",
                    "Created sections with gameplay videos and screenshots",
                    "Added pre-order system with payment gateway integration",
                    "Optimized performance for fast loading"
                ]
            },
            project3: {
                title: "Logistics System — Buster GFC",
                desc: "Order management subsystem in automated logistics system.",
                problem: "A logistics company needed to automate the order management process. Manual order processing led to errors, delays and inefficient resource usage. A system was needed to track orders from creation to delivery.",
                whatWeDid: [
                    "Developed full-featured order management system",
                    "Implemented automatic order distribution across warehouses and couriers",
                    "Created real-time order status tracking system",
                    "Integrated shipping company APIs for automatic waybill creation",
                    "Implemented notification system for clients and employees",
                    "Developed analytics dashboard for optimizing logistics processes"
                ]
            }
        }
    }
};


